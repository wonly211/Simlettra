...
function sendQueueStatusLabel(status: SendOperationResult['queue']['status']): string {
  return {
    not_created: '未创建队列任务',
    pending: '等待 Queue 执行',
    running: 'Queue 正在执行',
    retry_wait: 'Queue 等待重试',
    needs_attention: 'Queue 需要检查',
    succeeded: 'Queue 已成功',
    cancelled: 'Queue 已取消',
  }[status]
}

function sendQueueErrorLabel(errorCode: string | null): string {
  if (errorCode === 'queue_binding_missing') return '未发现 Queue 绑定，请检查部署配置'
  if (errorCode === 'queue_enqueue_failed') return 'Queue 入队失败，任务等待自动补投'
  return errorCode || ''
}

function sendProviderLabel(
  providerType: SendOperationResult['recipients'][number]['providerType'],
): string {
  if (providerType === 'smtp2go') return 'SMTP2GO'
  if (providerType === 'resend') return 'Resend'
  return '尚未调用'
}

function sendProviderResultLabel(
  result: SendOperationResult['recipients'][number]['providerResult'],
): string {
  if (result === 'accepted') return '已接受'
  if (result === 'not_accepted') return '明确拒绝'
  if (result === 'unknown') return '结果未知'
  return '尚未获得结果'
}
          ...
          <template v-else>
            ...
            <article class="draft-editor-panel">
              ...
              <div v-else-if="lastSendOperation" class="draft-send-result" aria-live="polite">
                ...
                <dl class="send-result-diagnostics">
                  <div>
                    <dt>Queue 任务</dt>
                    <dd>
                      {{ sendQueueStatusLabel(lastSendOperation.queue.status) }}
                      <span v-if="lastSendOperation.queue.attemptCount > 0">
                        · 第 {{ lastSendOperation.queue.attemptCount }} 次尝试
                      </span>
                    </dd>
                  </div>
                  <div v-if="lastSendOperation.queue.errorCode">
                    <dt>Queue 状态说明</dt>
                    <dd>
                      {{ sendQueueErrorLabel(lastSendOperation.queue.errorCode) }}
                      <small>（代码：{{ lastSendOperation.queue.errorCode }}）</small>
                    </dd>
                  </div>
                </dl>
                <div class="send-recipient-list">
                  <article v-for="recipient in lastSendOperation.recipients" :key="recipient.id">
                    <div>
                      ...
                      <small v-if="recipient.channel === 'external'">
                        Provider：{{ sendProviderLabel(recipient.providerType) }} · 请求结果：{{
                          sendProviderResultLabel(recipient.providerResult)
                        }}
                      </small>
                      <small v-if="recipient.providerSubmissionId">
                        email_id：{{ recipient.providerSubmissionId }}
                      </small>
                      <small v-if="recipient.providerErrorCode">
                        Provider 错误代码：{{ recipient.providerErrorCode }}
                      </small>
                      <small>{{
            ...
            >
              ...
              <div v-if="outboundCallbackUrlNotice" class="outbound-callback-notice">
                ...
                <div class="copy-field outbound-callback-copy-field">
                  <input
                    :value="outboundCallbackUrlNotice"
                    readonly
                    aria-label="SMTP2GO 回调地址"
                  />
                  <button
                    class="button button--secondary button--compact"
                    type="button"
